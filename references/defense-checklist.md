...
30. **Gate consequential actions behind explicit confirmation** — Maps to `PI-NO-CONFIRM-GATE`. Delete, send, pay, publish, and deploy are privileged actions: require explicit user confirmation before each one, make destructive actions staged or reversible (soft-delete with an undo window beats a hard delete behind a dialog), and honor stop requests immediately — mid-action if necessary. State the gate in the prompt so intent is auditable, but enforce it outside the model: the OpenClaw inbox-deletion incident (2026-02-23) and the adaptive-attack literature both show that in-model politeness is not a control. The gate belongs on consequential actions only — gating every read trains users to click through the gate that matters.
